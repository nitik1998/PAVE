"""Aggregate outputs/tables_test/<method>_overall.csv into a single figure.

Mirrors summarize_probes.py but pulls from the test-split table directory.
"""
...
def main(tables_root: str, out_fig: str, out_md: str, title_suffix: str):
    log = logging.getLogger("summarize_test")
    methods = ["dinov2", "dinov2_448", "dinov2_448_full", "dinov2_large", "siglip2", "openpi_siglip", "florence2", "random_features"]
    rows = []
    for m in methods:
    ...
    bars = ax.bar(xs, miou, color="#c94a72")
    ax.set_xticks(xs)
    ax.set_xticklabels([r["method"] for r in rows], rotation=15, ha="right")
    ax.set_ylabel("mIoU (UMD test subset)")
    ax.set_ylim(0, max(0.6, max(miou) * 1.2))
    for b, v in zip(bars, miou):
        ax.text(b.get_x() + b.get_width() / 2, v + 0.005, f"{v:.3f}", ha="center", fontsize=9)
    ax.set_title("Cross-method affordance mIoU on UMD test split " + title_suffix)
    fig.tight_layout()
    ...
    ax2.set_ylabel("IoU")
    ax2.set_title("Per-class IoU on UMD test split")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tables-root", default="outputs/tables_test")
    ap.add_argument("--out-fig", default="outputs/figures/probe_miou_test.png")
    ap.add_argument("--out-md", default="outputs/tables_test/probe_summary.md")
    ap.add_argument("--title-suffix", default="(unseen images)")
    args = ap.parse_args()
    main(args.tables_root, args.out_fig, args.out_md, args.title_suffix)
